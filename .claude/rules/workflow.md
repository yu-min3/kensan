...
- **フロントエンド変更時**: `npm run build` で型チェックを実行（TSエラーがないことを確認）。
...
- **新しいコンポーネント/ストア/ページ追加** → `src/ARCHITECTURE.md` の該当セクション更新
- **DB スキーマ変更** → `backend/ARCHITECTURE.md` のデータベースセクション更新
- **新しいルート追加** → `src/ARCHITECTURE.md` のルーティングセクション更新
...
- [ ] 変更したフロントエンドのビルド確認 (`npm run build`) — TS エラーがないこと
