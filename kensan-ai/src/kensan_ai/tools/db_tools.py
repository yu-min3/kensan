...
def _parse_date(value: str | None) -> date | None:
    """Parse an ISO date string, returning None if invalid or empty."""
    if not value:
        return None
    try:
...
def _parse_time(value: str | None) -> time | None:
    """Parse a time string (HH:MM), returning None if invalid or empty."""
    if not value:
        return None
    try:
        parts = value.split(":")
...
async def get_tasks(args: dict[str, Any]) -> dict[str, Any]:
    ...
    tasks = await db_get_tasks(
        user_id=user_id,
        milestone_id=_parse_uuid(args.get("milestone_id")),
...
async def create_time_block(args: dict[str, Any]) -> dict[str, Any]:
    ...
    end_time = _parse_time(args.get("end_time"))
    ...
    block = await db_create_time_block(
        ...
        goal_id=_parse_uuid(args.get("goal_id")),
