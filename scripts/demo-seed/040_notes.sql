-- ============================================================================
-- Demo Seed: Notes (日記 ~27件 + 学習記録 ~12件) + note_tags
-- ============================================================================
...
('dd400025-0000-0000-0000-000000000000', 'dddddddd-dddd-dddd-dddd-dddddddddddd', 'diary',
 '本番障害から学んだこと',
 E'# 本番障害から学んだこと\n\n昨日の本番障害、原因はgoroutineリークだった。\n\nHTTPクライアントのレスポンスをcloseしてないgoroutineが溜まって、\nメモリ使用量がじわじわ上昇。OOMKillerに殺されてた。\n\n## 対応\n- `context.WithTimeout`を追加してタイムアウト制御\n- `defer resp.Body.Close()`を全箇所に追加\n- メモリ使用量のアラート閾値を追加\n\n## 感じたこと\n先月勉強したGo並行処理パターンがそのまま活きた。\n最初は焦ったけど、原因特定は30分くらいでできた。\n\n佐藤さんに「落ち着いて対応できてたね」と言われて嬉しかった。\nテックリードに近づいてる実感がある。\n\n勉強 → 実務で使える → 自信がつく、のサイクルが回り始めた。',
 CURRENT_DATE,
 'dd000001-0000-0000-0000-000000000000', 'テックリード昇格', '#3B82F6',
 CURRENT_DATE, CURRENT_DATE);

-- CURRENT_DATE - 2: MVPデプロイ成功
INSERT INTO notes (id, user_id, type, title, content, date,
    goal_id, goal_name, goal_color,
    created_at, updated_at) VALUES
('dd400026-0000-0000-0000-000000000000', 'dddddddd-dddd-dddd-dddd-dddddddddddd', 'diary',
 'MVPデプロイ成功！',
 E'# MVPデプロイ成功！\n\nfly.ioに家計簿アプリを初デプロイした。動いた！\n\n## やったこと\n- Dockerfileのマルチステージビルド\n- fly.tomlの設定\n- PostgreSQLのアドオン設定\n- 環境変数の管理\n\n## ハマったこと\n- GoのCGO_ENABLEDを0にしないとAlpineで動かない\n- fly.ioのヘルスチェックパスの設定忘れてて起動失敗\n- PostgreSQLの接続文字列のsslmodeがrequireじゃないとダメ\n\n## 次のステップ\n- Zenn記事にデプロイ手順をまとめる\n- CI/CDでauto deployを設定\n- ドメイン設定（まだfly.devのサブドメイン）\n\n自分が書いたコードがインターネット上で動いてるの、\nSIer時代にはなかった感動がある。',
 CURRENT_DATE - 2,
 'dd000002-0000-0000-0000-000000000000', '個人開発アプリリリース', '#10B981',
 CURRENT_DATE - 2, CURRENT_DATE - 2);

-- ==============================================================================
-- 学習記録 (learning type) — ~12件
-- ==============================================================================
INSERT INTO notes (id, user_id, type, title, content, date,
...
-- CURRENT_DATE - 1: AWSコスト最適化 実戦パターン集
INSERT INTO notes (id, user_id, type, title, content, date,
    task_id, milestone_id, milestone_name, goal_id, goal_name, goal_color,
    created_at, updated_at) VALUES
('dd500011-0000-0000-0000-000000000000', 'dddddddd-dddd-dddd-dddd-dddddddddddd', 'learning',
 'AWSコスト最適化 実戦パターン集',
 E'# AWSコスト最適化 実戦パターン集\n\n## 模擬試験弱点の集中対策（2日目）\n\n### 計算問題パターン\n\n#### パターン1: RI vs オンデマンド損益分岐点\n- 1年RI: 月額$500（前払いなし）/ オンデマンド: 月額$730\n- 年間節約: ($730-$500)*12 = $2,760\n- 前払いありだと更にお得だが、キャッシュフローとのトレードオフ\n\n#### パターン2: Savings Plans計算\n- Compute Savings Plans: EC2, Fargate, Lambda横断で適用\n- EC2 Instance Savings Plans: 特定ファミリー限定だが割引率高い\n- 「$10/hour のコミットメント」→ まず高割引率で適用、超過分はオンデマンド\n\n#### パターン3: S3ストレージクラス選択\n- 30日以内にアクセス多い → S3 Standard\n- 30日後アクセス減る → S3 IA（取り出し課金注意）\n- 90日+ → Glacier Flexible Retrieval\n- アクセスパターン不明 → Intelligent-Tiering（自動移行）\n\n### 正答率の推移\n- 1日目: 45%（ヤバい）\n- 2日目: 58%（パターン掴めてきた）\n- 目標: 65%以上',
 CURRENT_DATE - 1,
 'dd100014-0000-0000-0000-000000000000', 'dd030001-0000-0000-0000-000000000000', '模擬試験80%以上',
 'dd000003-0000-0000-0000-000000000000', 'AWS SAP取得', '#F59E0B',
 CURRENT_DATE - 1, CURRENT_DATE - 1);

-- CURRENT_DATE - 2: fly.io デプロイメモ
INSERT INTO notes (id, user_id, type, title, content, date,
    task_id, milestone_id, milestone_name, goal_id, goal_name, goal_color,
    created_at, updated_at) VALUES
('dd500012-0000-0000-0000-000000000000', 'dddddddd-dddd-dddd-dddd-dddddddddddd', 'learning',
 'fly.io デプロイメモ',
 E'# fly.io デプロイメモ\n\n## 手順\n\n### 1. flyctl セットアップ\n```bash\ncurl -L https://fly.io/install.sh | sh\nflyctl auth login\nflyctl launch  # fly.toml 生成\n```\n\n### 2. Dockerfile（マルチステージビルド）\n```dockerfile\n# Build stage\nFROM golang:1.22-alpine AS builder\nWORKDIR /app\nCOPY go.mod go.sum ./\nRUN go mod download\nCOPY . .\nRUN CGO_ENABLED=0 go build -o main ./cmd/main.go\n\n# Run stage\nFROM alpine:3.19\nCOPY --from=builder /app/main /main\nEXPOSE 8080\nCMD [\"/main\"]\n```\n\n### 3. PostgreSQL\n```bash\nflyctl postgres create --name myapp-db\nflyctl postgres attach myapp-db\n```\n\n### 4. ハマりポイント\n- `CGO_ENABLED=0` 必須（Alpine環境）\n- ヘルスチェック: `fly.toml` に `[http_service]` セクション\n- PostgreSQL: `sslmode=require` が必要\n- メモリ: 256MB だとギリギリ。512MB推奨\n\n### 5. CI/CD（GitHub Actions）\n```yaml\n- uses: superfly/flyctl-actions/setup-flyctl@master\n- run: flyctl deploy --remote-only\n```',
 CURRENT_DATE - 2,
 'dd100009-0000-0000-0000-000000000000', 'dd020003-0000-0000-0000-000000000000', '本番デプロイ',
 'dd000002-0000-0000-0000-000000000000', '個人開発アプリリリース', '#10B981',
 CURRENT_DATE - 2, CURRENT_DATE - 2);


-- ==============================================================================
-- note_tags (30-40件) — ノートとタグの関連付け
-- ==============================================================================
-- Tags:
-- dd0a0001: 開発, dd0a0002: 学習, dd0a0003: 資格,
-- dd0a0004: 運動, dd0a0005: 読書

INSERT INTO note_tags (note_id, tag_id) VALUES
-- 日記 → タグ
('dd400001-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- 個人開発始動 → 開発
('dd400002-0000-0000-0000-000000000000', 'dd0a0003-0000-0000-0000-000000000000'), -- 朝活チャレンジ → 資格
('dd400004-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- DB設計完了 → 開発
('dd400006-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- API実装楽しい → 開発
('dd400007-0000-0000-0000-000000000000', 'dd0a0003-0000-0000-0000-000000000000'), -- AWS模擬試験60% → 資格
('dd400009-0000-0000-0000-000000000000', 'dd0a0002-0000-0000-0000-000000000000'), -- Go並行処理 → 学習
('dd400009-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- Go並行処理 → 開発
('dd400010-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- CI/CD改善 → 開発
('dd400011-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- React難しい → 開発
('dd400011-0000-0000-0000-000000000000', 'dd0a0002-0000-0000-0000-000000000000'), -- React難しい → 学習
('dd400013-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- useState/useReducer → 開発
('dd400013-0000-0000-0000-000000000000', 'dd0a0002-0000-0000-0000-000000000000'), -- useState/useReducer → 学習
('dd400014-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- エラーハンドリング → 開発
('dd400016-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- MVP完成が見えてきた → 開発
('dd400017-0000-0000-0000-000000000000', 'dd0a0004-0000-0000-0000-000000000000'), -- ジムが減ってる → 運動
('dd400018-0000-0000-0000-000000000000', 'dd0a0003-0000-0000-0000-000000000000'), -- AWS模擬試験72% → 資格
('dd400019-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- Zenn記事 → 開発
('dd400022-0000-0000-0000-000000000000', 'dd0a0005-0000-0000-0000-000000000000'), -- Clean Architecture読了 → 読書
('dd400022-0000-0000-0000-000000000000', 'dd0a0002-0000-0000-0000-000000000000'), -- Clean Architecture読了 → 学習
('dd400023-0000-0000-0000-000000000000', 'dd0a0002-0000-0000-0000-000000000000'), -- Go context → 学習
('dd400023-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- Go context → 開発
('dd400024-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- Tailwind CSS → 開発
('dd400025-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- 本番障害 → 開発
('dd400025-0000-0000-0000-000000000000', 'dd0a0002-0000-0000-0000-000000000000'), -- 本番障害 → 学習
('dd400026-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- MVPデプロイ → 開発

-- 学習記録 → タグ
('dd500001-0000-0000-0000-000000000000', 'dd0a0002-0000-0000-0000-000000000000'), -- Go並行処理 → 学習
('dd500001-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- Go並行処理 → 開発
('dd500002-0000-0000-0000-000000000000', 'dd0a0005-0000-0000-0000-000000000000'), -- Clean Architecture → 読書
('dd500002-0000-0000-0000-000000000000', 'dd0a0002-0000-0000-0000-000000000000'), -- Clean Architecture → 学習
('dd500003-0000-0000-0000-000000000000', 'dd0a0003-0000-0000-0000-000000000000'), -- AWS模擬試験1回目 → 資格
('dd500004-0000-0000-0000-000000000000', 'dd0a0002-0000-0000-0000-000000000000'), -- React hooks → 学習
('dd500004-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- React hooks → 開発
('dd500005-0000-0000-0000-000000000000', 'dd0a0005-0000-0000-0000-000000000000'), -- DDD入門 → 読書
('dd500005-0000-0000-0000-000000000000', 'dd0a0002-0000-0000-0000-000000000000'), -- DDD入門 → 学習
('dd500006-0000-0000-0000-000000000000', 'dd0a0003-0000-0000-0000-000000000000'), -- AWS模擬試験2回目 → 資格
('dd500007-0000-0000-0000-000000000000', 'dd0a0002-0000-0000-0000-000000000000'), -- Goエラーハンドリング → 学習
('dd500007-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- Goエラーハンドリング → 開発
('dd500008-0000-0000-0000-000000000000', 'dd0a0003-0000-0000-0000-000000000000'), -- AWS VPC設計 → 資格
('dd500008-0000-0000-0000-000000000000', 'dd0a0002-0000-0000-0000-000000000000'), -- AWS VPC設計 → 学習
('dd500009-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'), -- DB設計ドキュメント → 開発
('dd500010-0000-0000-0000-000000000000', 'dd0a0003-0000-0000-0000-000000000000'), -- AWSコスト最適化 → 資格
('dd500010-0000-0000-0000-000000000000', 'dd0a0002-0000-0000-0000-000000000000'), -- AWSコスト最適化 → 学習
('dd500011-0000-0000-0000-000000000000', 'dd0a0003-0000-0000-0000-000000000000'), -- AWSコスト実戦パターン → 資格
('dd500011-0000-0000-0000-000000000000', 'dd0a0002-0000-0000-0000-000000000000'), -- AWSコスト実戦パターン → 学習
('dd500012-0000-0000-0000-000000000000', 'dd0a0001-0000-0000-0000-000000000000'); -- fly.ioデプロイメモ → 開発
