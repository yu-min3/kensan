...
export function TimeblockProposalCard({ card }: { card: BriefingCardData }) {
  const [approving, setApproving] = useState(false)
  const conversationId = useBriefingStore((s) => s.conversationId)
  const updateCard = useBriefingStore((s) => s.updateCard)
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      updateCard(card.type, {
        status: 'action_done',
        actionStatus: 'approved',
      })
    } catch {
      // Keep action_pending on error
    } finally {
      setApproving(false)
    }
  }
      ...
      <div className="space-y-3">
        {actions.length > 0 ? (
          <>
            ...
            {card.actionStatus === 'approved' ? (
              <div className="flex items-center gap-1.5 text-sm text-emerald-600 dark:text-emerald-400">
                <Check className="h-4 w-4" />
                <span>作成しました</span>
              </div>
            ) : card.actionStatus === 'skipped' ? (
