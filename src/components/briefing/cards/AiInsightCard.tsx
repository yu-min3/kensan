...
export function AiInsightCard({ card, mode }: AiInsightCardProps) {
  const insightText = useBriefingStore((s) => s.insightText)
  const [diaryDraft, setDiaryDraft] = useState('')
  const [saved, setSaved] = useState(false)
  const navigate = useNavigate()

  // In evening mode, the insight text includes a learning diary draft
  ...
  // Initialize diary draft from AI-generated content
  const effectiveDraft = diaryDraft || diarySection

  const handleSaveDiary = () => {
    // In real implementation, this would call create_note API
    setSaved(true)
  }
