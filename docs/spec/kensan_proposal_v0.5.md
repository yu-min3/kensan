...
| **Argo Workflows** | ML Pipeline | モデル訓練・評価パイプライン |
| **Argo Rollouts** | 全マイクロサービス | カナリアデプロイ、Progressive Delivery |
| **ArgoCD** | Platform | GitOpsデプロイメント（既存） |

### 7.5 MLOps基盤（Phase 4+ / 拡張案）

ポートフォリオとしての価値向上のため、Kubernetes上でのMLOpsワークフローを構築する。個人利用では学習データが限られるため、**スケールを前提とした設計**を先行して構築し、将来的なマルチユーザー対応やOSSコミュニティでのデータ蓄積に備える。

#### 7.5.1 MLOps技術スタック

```
┌─────────────────────────────────────────────────────────────┐
│                  Kensan MLOps Architecture                  │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ┌─────────────────┐    ┌─────────────────┐                │
│  │  Argo Workflows │    │     MLflow      │                │
...
│           ▼    ▼                                            │
│  ┌─────────────────┐    ┌─────────────────┐                │
│  │ MLflow Serving  │ or │     KServe      │                │
│  │  (lightweight)  │    │ (Istio連携)     │                │
│  └─────────────────┘    └─────────────────┘                │
│                                                             │
│  ※ サービング選定は運用性・Istio連携の必要性で判断          │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
| コンポーネント | 役割 | 選定理由 |
|---------------|------|----------|
| **Argo Workflows** | ML Pipeline | 既存活用、Argo Events連携 |
| **MLflow** | 実験管理、モデルレジストリ | 業界標準、軽量 |
| **MLflow Serving / KServe** | モデルサービング | 運用性で選定 |

**サービング選定の比較**:

| 観点 | MLflow Serving | KServe |
|------|----------------|--------|
| 導入の容易さ | ◎ MLflow統合済み | △ 別途構築必要 |
| Istio連携 | △ 手動設定 | ◎ ネイティブ対応 |
| カナリアリリース | △ 要カスタム | ◎ Argo Rollouts連携可 |
| スケーラビリティ | ○ 十分 | ◎ Knative基盤 |
| 学習コスト | ○ 低い | ◎ 業界標準で価値高 |

→ MVP: MLflow Serving（シンプル）、将来: KServe移行検討（ポートフォリオ価値）

#### 7.5.2 ML活用機能（拡張案）

| 機能 | 概要 | 必要データ量 | 優先度 |
|------|------|-------------|--------|
| タスク時間予測 | 過去実績から所要時間を予測 | 中（100+記録） | ★★☆ |
| 目標達成予測 | 週末時点での達成確率 | 中（数週間分） | ★★☆ |
| 最適時間帯提案 | タスク種別×時間帯の生産性分析 | 大（数ヶ月分） | ★☆☆ |

#### 7.5.3 学習データ設計（スケール前提）

個人利用では学習データが限られるため、将来のスケールを見据えた特徴量設計を行う。

**基本特徴量**（Clockifyデータから取得）:
- プロジェクト / タスクカテゴリ
- 曜日 / 時間帯
- 過去の同タスク平均時間
- 直近の作業パターン
...
interface UserStatus {
  ...
  // 体調・コンディション
  energyLevel?: 1 | 2 | 3 | 4 | 5;      // 体調スコア
  sleepQuality?: 1 | 2 | 3 | 4 | 5;     // 睡眠の質
  sleepHours?: number;                   // 睡眠時間
  
  // 外部要因
  hasChildcare?: boolean;                // 育児担当日
  hasExternalMeeting?: boolean;          // 外部MTGあり
  workLocation?: 'office' | 'home' | 'other';
  
  // 主観的指標
  motivation?: 1 | 2 | 3 | 4 | 5;       // やる気
  focusLevel?: 1 | 2 | 3 | 4 | 5;       // 集中度（振り返り時）
}
```
...
```yaml
# Argo Workflow: 週次モデル再訓練
schedule: "0 3 * * 0"  # 毎週日曜 3:00
...
| スキル | Kensanでの実証 |
|--------|---------------|
| ML Pipeline構築 | Argo WorkflowsでE2Eパイプライン |
| 実験管理 | MLflowでのTracking/Registry運用 |
| モデルサービング | KServe or MLflow Servingでの推論API |
| MLOps監視 | Prometheusでのモデルメトリクス |
| Feature Engineering | 時系列+ユーザーコンテキストの特徴量設計 |
...
### Phase 4+: MLOps基盤（拡張）
- MLflow導入（実験管理、モデルレジストリ）
- Argo WorkflowsによるMLパイプライン構築
- タスク時間予測モデルの実装
- MLflow Serving / KServe によるモデルサービング
- ユーザーステータス収集機能（オプトイン）
...
- **MLOps基盤構築経験**（MLflow + Argo Workflows）
