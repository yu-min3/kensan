...
func main() {
	...
	// Override port for record-service
	...
	recordRepo := repository.NewPostgresRepository(pool)
	recordService := service.New(recordRepo)
	...
	r.Get("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`{"status":"healthy","service":"record-service"}`))
	})

	// API routes
	r.Route("/api/v1", func(r chi.Router) {
		recordHandler.RegisterRoutes(r, middleware.Auth(jwtManager))
	})
