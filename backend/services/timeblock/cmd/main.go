...
func main() {
	...
	// Override default port for timeblock service
	...
	timeblockRepo := repository.NewPostgresRepository(pool)
	...
	r.Get("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`{"status":"healthy","service":"timeblock-service"}`))
	})

	// API routes (auth required)
	r.Route("/api/v1", func(r chi.Router) {
		r.Use(middleware.Auth(jwtManager))
		timeblockHandler.RegisterRoutes(r)
	})
	...
	go func() {
		log.Info().Str("addr", addr).Msg("Starting timeblock service")
