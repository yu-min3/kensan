...
func main() {
	...
	// Override port for diary-service
	...
	diaryRepo := repository.NewPostgresRepository(pool)
	diaryService := service.New(diaryRepo)
	diaryHandler := handler.New(diaryService)
	...
	r.Get("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`{"status":"healthy","service":"diary-service"}`))
	})

	// API routes
	r.Route("/api/v1", func(r chi.Router) {
		diaryHandler.RegisterRoutes(r, middleware.Auth(jwtManager))
	})
